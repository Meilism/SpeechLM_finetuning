...
def eval_ctc_model(
    ...
    if os.path.isdir(model):
        ckpt = os.path.join(model, checkpoint_file)
        # eval_log_file = os.path.join(ckpt, 'eval.log')
        eval_log_file = None
        if save_results:
            results_path = os.path.join(model, "decode", input_type, lm.replace("/", "_"))
        else:
            results_path = None
        emission_path = (
    ...
    cmd = (
        f"python -m speechlm.eval.infer_asr {data}"
        f" --user-dir {user_dir}"
    ...
    if lm == "nolm":
        ...
        cmd += f" --w2l-decoder kenlm --lm-model {lmdir}/4gram.bin --lexicon {lmdir}/lexicon.lst"

    cmd += f" --task-cfg {user_dir}/config/slue/speechlm_base_{input_type}_ner_finetune.yaml"
    if input_type == "text":
        cmd += f" --text-data {text_data}"
    ...
    msg = f"{subset},{model},{lm},{input_type},{model_size},{extract_size},{time_used},{wer}"
    if not quiet:
        print(msg)
    if csv_log_file is not None:
        with open(os.path.join(model, "decode", csv_log_file), "a") as f:
            print(msg, file=f)
