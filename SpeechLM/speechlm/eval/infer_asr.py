...
from speechlm.tasks.sequence_labeling import AudioLabelingConfig, TextLabelingConfig, AudioLabelingTask, TextLabelingTask
...
def add_task_argument(parser):
    parser.add_argument(
        "--task-cfg",
        type=str,
        help="task cfg that matches the modality of input data",
    )
    parser.add_argument(
        "--text-data",
        type=str,
...
def main(args, task=None, model_state=None):
    ...
    if args.load_emissions:
        ...
        # task = tasks.setup_task(args)
        if args.task_cfg:
            task_cfg = OmegaConf.load(args.task_cfg).task
...
def make_parser():
    ...
    parser = add_task_argument(parser)
    return parser
